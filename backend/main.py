...
app = FastAPI(
    ...
    description=(
        ...
        "concentration index, and tier-level weighted performance. "
...
# ---------------------------------------------------------------------------
# If PORTFOLIO_API_KEYS is set (comma-separated), requests must supply a valid
# `X-API-Key` header. Unset (the default) = open access for local development.
# In production, keys are issued per-customer on successful Stripe checkout;
...
async def portfolio_summary(payload: PortfolioPayload) -> dict:
    """Full analytics report: exposure, PnL, HHI, tier metrics, and holdings detail."""
...
async def risk_metrics(payload: PortfolioPayload) -> dict:
    """Concentration risk metrics only (HHI, classification, effective positions)."""
