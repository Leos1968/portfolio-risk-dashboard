"""IB/PE Modeling Suite — three-statement, M&A accretion/dilution, and LBO.

A standalone Streamlit app (deployed separately from the risk dashboards in
../frontend and ../analyzer) that builds the models an investment-banking or
private-equity analyst actually lives in:

    📐 Three-Statement Model   — driver-based IS/BS/CFS that links and *balances*
    🤝 M&A / Accretion-Dilution — does the deal add to or dilute EPS?
    🏦 LBO / Returns            — leverage, cash sweep, exit, IRR & MOIC
    🧭 Methodology              — how it links, honest limits, glossary

All math lives in pure, unit-tested engines (three_statement.py, ma_model.py,
lbo_model.py); this file is presentation only. No market data, no API keys —
every number is computed live from the assumptions on screen.
"""
...
from three_statement import Assumptions, BaseFinancials, build_model
from ma_model import Acquirer, DealTerms, Target, run_deal, sensitivity_premium_stock, \
    sensitivity_synergies_premium
from lbo_model import LBOAssumptions, run_lbo, irr_sensitivity
from presets import THREE_STATEMENT_PRESETS, MA_PRESETS, LBO_PRESETS
...
st.sidebar.markdown(
    f"**Explore more**\n\n"
    f"[📊 Market Command Center (sister app)](https://market-command-dashboard.streamlit.app)  \n"
...
st.sidebar.caption("Educational tool — not investment advice. All figures in the currency you enter (e.g. $ millions).")
